 // ──────────────────────────────────────
        ...
        function handleLogin() {
            ...
            if (!isValidEmail(email)) { showAlert('Email không hợp lệ.'); return; }
            if (!password) { showAlert('Vui lòng nhập mật khẩu.'); return; }
            if (password.length < 3) { showAlert('Mật khẩu quá ngắn.'); return; }
            ...
            setTimeout(() => {
                ...
                if (found) {
                    // Registered user
                    localStorage.setItem('loggedInUser', JSON.stringify({ email: found.email, name: found.name, role: 'user' }));
                    showAlert('Đăng nhập thành công! Đang chuyển trang...', 'success');
                    setTimeout(() => { window.location.href = 'index.html'; }, 800);
                } else if (currentRole === 'admin') {
                    // Admin demo
                    localStorage.setItem('loggedInUser', JSON.stringify({ email, name: 'Admin', role: 'admin' }));
                    showAlert('Đăng nhập admin thành công!', 'success');
                    setTimeout(() => { window.location.href = 'admin.html'; }, 800);
                } else {
                    // User demo — any email/password works
                    localStorage.setItem('loggedInUser', JSON.stringify({ email, name: email.split('@')[0], role: 'user' }));
        ...
        function handleRegister() {
            ...
            if (password !== confirm) { showAlert('Mật khẩu xác nhận không khớp.'); return; }

            const users = JSON.parse(localStorage.getItem('studyNoteUsers') || '[]');
            if (users.find(u => u.email === email)) {
                showAlert('Email này đã được đăng ký.'); return;
            }
            ...
            setTimeout(() => {
                ...
                localStorage.setItem('loggedInUser', JSON.stringify({ email, name, role: 'user' }));
