...
function initStatCounters() {
    const stats = {
        totalUsers: { el: document.querySelector('.stat-card:nth-child(1) .stat-value'), target: 12847, suffix: '' },
        totalNotes: { el: document.querySelector('.stat-card:nth-child(2) .stat-value'), target: 45231, suffix: '' },
        uploads:    { el: document.querySelector('.stat-card:nth-child(3) .stat-value'), target: 8549, suffix: '' },
        downloads:  { el: document.querySelector('.stat-card:nth-child(4) .stat-value'), target: 93210, suffix: '' },
        active:     { el: document.querySelector('.stat-card:nth-child(5) .stat-value'), target: 1243, suffix: '' },
        storage:    { el: document.querySelector('.stat-card:nth-child(6) .stat-value'), target: 642, suffix: ' GB' },
    };
    ...
    // Stat change labels
    const changes = ['+8.2% tuần này', '+12.5% tuần này', '+5.1% tuần này', '+18.3% tuần này', '+3.7% hôm nay', ''];
    document.querySelectorAll('.stat-change').forEach((el, i) => {
        if (changes[i]) el.innerHTML = `<i class="fas fa-arrow-up"></i> ${changes[i]}`;
    });

    // Category counts
    const categoryCounts = ['2,341 ghi chú', '1,892 ghi chú', '3,120 ghi chú', '1,540 ghi chú', '2,780 ghi chú'];
    document.querySelectorAll('.category-count').forEach((el, i) => {
        if (categoryCounts[i]) el.textContent = categoryCounts[i];
    });

    // Legend values
    const legendValues = [' Người dùng hoạt động: 8,923', ' Người dùng không hoạt động: 2,681', ' Người dùng mới: 1,243'];
...
let overviewChartInstance, userActivityChartInstance, storageChartInstance, uploadChartInstance, downloadChartInstance;
...
function initOverviewChart() {
    ...
    const data = [820, 1200, 950, 1540, 1320, 1890, 2100, 1750, 2340, 2560, 2100, 2847];
    ...
    overviewChartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                ...
                y: { grid: { color: c.gridColor }, ticks: { color: c.textColor, font: { size: 11 } } }
            }
        }
    });
    ...
    if (select) {
        select.addEventListener('change', () => {
            const datasets = {
                'Người dùng': [820, 1200, 950, 1540, 1320, 1890, 2100, 1750, 2340, 2560, 2100, 2847],
                'Ghi chú':    [1200, 1800, 1400, 2100, 1900, 2600, 3000, 2400, 3200, 3500, 2900, 3800],
                'Hoạt động':  [3000, 4200, 3600, 5100, 4700, 6200, 7100, 5900, 7800, 8500, 7100, 9300],
            };
            overviewChartInstance.data.datasets[0].data = datasets[select.value] || datasets['Người dùng'];
